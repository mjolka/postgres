...
static bool
numutils_isdigit(const char c)
{
	return c >= '0' && c <= '9';
}

static bool
numutils_isspace(const char c)
{
	if (c == ' ' ||
		c == '\t' ||
		c == '\n' ||
...
pg_atoi(const char *s, int size, int c)
{
	 ...
	 */
	while (*badp && *badp != c && numutils_isspace(*badp))
		badp++;
...
pg_strtoint16(const char *s)
{
	...
	if (unlikely(!numutils_isdigit(*ptr)))
		goto invalid_syntax;
